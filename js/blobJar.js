...
function add_blob(ref) {
  ...
  dy[ref] = (0.5 + Math.random()) * (Math.random() > 0.5 ? 2 : -2);
  dx[ref] = (0.5 + Math.random()) * (Math.random() > 0.5 ? 2 : -2);
  dz[ref] = (0.5 + Math.random()) * (Math.random() > 0.5 ? 0.2 : -0.2);
  blob[ref] = dv;
...
function checkity(j) {
  ...
  if (j)
    for (var i = 0; i < j; i++) {
      if (
        Math.abs(xpos[i] - xpos[j]) < size &&
        Math.abs(ypos[i] - ypos[j]) < size
      )
        r = true;
    }
  return r;
...
function jamjar(ref) {
  var i;
  if (ypos[ref] + dy[ref] < -size / 2 || ypos[ref] + dy[ref] > shigh + size / 2)
    rejig(ref, "y");
  else if (
    xpos[ref] + dx[ref] < -size / 2 ||
    xpos[ref] + dx[ref] > swide + size / 2
  )
    rejig(ref, "x");
  else if (
    Math.abs(ypos[ref] + dy[ref] - y) < size / 2 &&
    Math.abs(xpos[ref] + dx[ref] - x) < size / 2 &&
    zpos[ref] < size / 20
  ) {
    if (Math.abs(ypos[ref] + dy[ref] - y) > Math.abs(xpos[ref] + dx[ref] - x))
      dy[ref] = -dy[ref];
    else dx[ref] = -dx[ref];
  } else
    for (i = 0; i < blobs; i++) {
      if (i == ref) continue;
      if (
        Math.abs(ypos[ref] + dy[ref] - ypos[i]) < size / 2 &&
        Math.abs(xpos[ref] + dx[ref] - xpos[i]) < size / 2 &&
        Math.abs(zpos[ref] - zpos[i]) < size / 20
      ) {
        if (
          Math.abs(ypos[ref] + dy[ref] - ypos[i]) >
          Math.abs(xpos[ref] + dx[ref] - xpos[i])
        )
          dy[ref] = -dy[ref];
        else dx[ref] = -dx[ref];
      }
